...
exports.login = async function (req, res) {
    try {
        ...
        let dispatcher = await Dispatcher.findOne({ email: email })
        if (!dispatcher) {
        ...
        res.json({ success: true, message: message, dispatcher: dispatcher })
    } catch (error) {
...
exports.forgot_password = async function (req, res) {
    try {
        ...
        let link = req.get('origin') + '/dispatcher/reset-password?id=' + id + '&&token=' + token;
        let html = ` <a href="${link}">Click Here</a>`
        allemails.userForgotPassword(req, dispatcher, html);
        await Dispatcher.findOneAndUpdate({ _id: id }, { token: token })
...
exports.update_password = async function (req, res) {
    try {
        ...
        let password = req.body.password
        let dispatcher = await Dispatcher.findOne({ _id: id, token: token })      
        if (!dispatcher) {
...
exports.delete = async function (req, res) {
    try {
        let params_array = [{ name: 'dispatcher_id', type: 'string' }]
        let response = await utils.check_request_params_async(req.body, params_array)
        if (!response.success) {
            res.json(response)
            return;
        ...
        // code 
        let dispatcher = await Dispatcher.findOne({ _id: req.body.dispatcher_id })
        ...
        await Dispatcher.findByIdAndDelete(req.body.dispatcher_id)
